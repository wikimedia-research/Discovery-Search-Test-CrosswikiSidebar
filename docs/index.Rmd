...
- affiliation: "Product Manager (Analysis, Search Frontend), Wikimedia Foundation"
  name: "Deb Tankersley"
abstract: "Some people seemed to click on the additional cross-wiki results that were shown to them."
output:
  pdf_document:
...
table_caps(name = "click_counts", caption = "Number of click events by group.")
table_caps(name = "sample_interactions", caption = "A sample of some visitors' interaction with the cross-wiki search results after they were enrolled into the test groups.")
figure_caps(name = "zero_results_rate", caption = "Zero results rate of experimental groups across wikis by type of results.")
figure_caps(name = "counts", caption = "Average number of searches, average number of search engine result pages (SERPs), total searches, total SERPs, and total sessions by group and wiki.")
...
On 9 February 2017 we deployed an A/B test on the desktop version of Catalan, Italian, Persian, and Polish Wikipedias to assess the efficacy of this feature. The test concluded on 22 February 2017, after a total 6592 search sessions have been anonymously tracked.

# Methods

Event logging was implemented in JavaScript according to the [TestSearchSatisfaction2 schema, revision 16270835](https://meta.wikimedia.org/w/index.php?title=Schema:TestSearchSatisfaction2&direction=next&oldid=15922352), data was stored in a MySQL database, and analyzed and reported using R.
...
See Phabricator ticket [T149806](https://phabricator.wikimedia.org/T149806) and Gerrit change [336896](https://gerrit.wikimedia.org/r/#/c/336896/) for full details of the implementation.
...
In `r table_caps("sessions_enrolled", display = "cite")`...

```{r click_counts_table, results='asis'}
searches %>%
  keep_where(event != "SERP") %>%
  group_by(Group = group, event) %>%
  dplyr::count() %>%
  tidyr::spread(event, n, fill = 0) %>%
  fable(format_caption(table_caps, "click_counts"))
```

In `r table_caps("click_counts", display = "cite")`...
...
searches %>%
  ...
  mutate(
    ...
    `Position of result` = vapply(`clicked-result position`, toOrdinal::toOrdinal, ""),
  ...
  select(User, Wikipedia, Group, Timestamp = timestamp,
         Click, `Position of result`,
         `Sister Project`, Destination = destination) %>%
  arrange(User, Timestamp) %>%
  fable(format_caption(table_caps, "sample_interactions"))
```

In `r table_caps("sample_interactions", display = "cite")`, we see the following three interactions:
...
In `r figure_caps("zero_results_rate", display = "cite")`...
...
In `r figure_caps("counts", display = "cite")`...
...
In `r figure_caps("clickthrough_rates", display = "cite")`...

## Cross-wiki Result Clicks
...
In `r table_caps("sister_clicks", display = "cite")`...
